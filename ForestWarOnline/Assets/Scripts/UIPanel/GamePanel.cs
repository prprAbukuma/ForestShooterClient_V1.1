...
public enum SyncHPState { Execute,StandBy }
...
public class GamePanel : BasePanel {
    ...
    private SyncHPState _syncHPState = SyncHPState.StandBy;

    public Button returnToRoomListBtn;//回到房间列表按钮
    public Button closeBtn;//游戏中途退出按钮【返回到房间列表】
    //血条相关
    public Slider p1HP_Slider;
    public Slider p2HP_Slider;
    private int p1HP;
    private int p2HP;
    ...
    private void Update()
    {
        ...
        #region 处理游戏中血条更新显示
        if (_syncHPState == SyncHPState.Execute)
        {
            //血条更新
            UpdateShowHPSlider();
            _syncHPState = SyncHPState.StandBy;
        }
        #endregion
    }
    /// <summary>
    ...
    /// 更新血条显示
    /// </summary>
    public void UpdateShowHPSlider()
    {
        p1HP_Slider.value = (float)p1HP / (float)100;
        p2HP_Slider.value = (float)p2HP / (float)100;
        Debug.LogWarning("血量更新了：P1hp" + p1HP + "P2Hp" + p2HP);
    }
    ...
    /// 处理刷新血条显示响应
    /// </summary>
    /// <param name="p1HP"></param>
    /// <param name="p2HP"></param>
    public void HandleSyncHPSlider(int p1HP,int p2HP)
    {
        this.p1HP = p1HP;
        this.p2HP = p2HP;
        //更改标志位后期交给Update处理主线程
        _syncHPState = SyncHPState.Execute;
    }
    /// <summary>
    /// 处理开始游戏请求的响应
    /// </summary>
